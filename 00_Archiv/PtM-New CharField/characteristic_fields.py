...
class CharField():
    ...
    def create_characteristic_fields(self):
        ...
        for i in range(0,charField_Absorption_Desorption_Methanolsynthesis.shape[0]):
            if charField_Absorption_Desorption_Methanolsynthesis[i,self.indexMoleFractionMethaneBiogasOut] >= self.param['constraints']['minMoleFractionCH4BiogasOut']:
                if charField_Absorption_Desorption_Methanolsynthesis[i,self.indexMoleFractionHydrogenSynthesisgas] / charField_Absorption_Desorption_Methanolsynthesis[i,self.indexMoleFractionCarbondioxideSynthesisgas] >= self.param['constraints']['minRatioH2_CO2_Synthesisgas']:
                    if charField_Absorption_Desorption_Methanolsynthesis[i,self.indexMoleFractionHydrogenSynthesisgas] / charField_Absorption_Desorption_Methanolsynthesis[i,self.indexMoleFractionCarbondioxideSynthesisgas] <= self.param['constraints']['maxRatioH2_CO2_Synthesisgas']:
                        opList.append(i+1)


        # Variable ranges for input variables and conversion values
        self.variablesList = []
        self.variablesList.append(opList)
        ...
        for i in self.variablesList[0]:
            if (i != 0):
                for k in range(0,self.lengthCharField_ABSDES):
                    self.charField_ABSDES[k][(i)] = charField_Absorption_Desorption_Methanolsynthesis[i-1,k]
        
                self.charField_ABSDES[self.lengthCharField_ABSDES+0][(i)] = (abs(charField_Absorption_Desorption_Methanolsynthesis[i-1,self.indexPowerPlantComponentsUnit1[0]]) \
                ...
                self.charField_ABSDES[self.lengthCharField_ABSDES+1][(i)] = self.param['biogas']['pressureIn'] / self.param['p0'] * self.charField_ABSDES[self.indexMassFlowBiogasIn][(i)] / self.param['biogas']['densityIn'] * self.param['T0'] / self.param['biogas']['temperatureIn']
                self.charField_ABSDES[self.lengthCharField_ABSDES+2][(i)] = self.param['biogas']['pressureOut'] / self.param['p0'] * self.charField_ABSDES[self.indexMassFlowBiogasOut][(i)] /  self.charField_ABSDES[self.indexDensityBiogasOut][(i)] * self.param['T0'] / self.param['biogas']['temperatureOut']

                
            else:
                self.charField_ABSDES[self.indexMassFlowHydrogenIn][(i)] = 0
                self.charField_ABSDES[self.indexMassFlowBiogasOut][(i)] = 0
                self.charField_ABSDES[self.indexDensityBiogasOut][(i)] = self.param['biogas']['densityIn']
                self.charField_ABSDES[self.indexMoleFractionMethaneBiogasOut][(i)] =  self.param['biogas']['minimumMoleFractionMethane']
                self.charField_ABSDES[self.indexMassFlowSynthesisgasIn][(i)] = 0
                self.charField_ABSDES[self.indexMoleFractionHydrogenSynthesisgas][(i)] = 0.75
                self.charField_ABSDES[self.indexMoleFractionCarbondioxideSynthesisgas][(i)] = 0.25
                self.charField_ABSDES[self.indexMassFlowMethanolWaterStorageIn][(i)] = 0
                self.charField_ABSDES[self.indexDensityMethanolWaterStorageIn][(i)] = self.param['storageMethanolWater']['InitialDensity'] = 804.5443
                self.charField_ABSDES[self.indexMassFlowMethanolWaterInCycle][(i)] = 250
                self.charField_ABSDES[self.indexMassFlowAdditionalHydrogen][(i)] = 0
                self.charField_ABSDES[self.lengthCharField_ABSDES+0][(i)] = 0
                self.charField_ABSDES[self.lengthCharField_ABSDES+1][(i)] = 0
                self.charField_ABSDES[self.lengthCharField_ABSDES+2][(i)] = 0
    ...
    def set_charField_output_ABSDES(self):
        ...
        self.massFlowBiogasIn = self.charField_ABSDES[self.indexMassFlowBiogasIn]  
