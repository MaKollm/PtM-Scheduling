...
def get_params(*args):
    ...
    param['methanol']['minimumAmountOpt'] = 21.2
    ...
    print(len(powerPriceHourly))

    #param['prices']['power'] = powerPriceHourly[0:param['controlParameters']['optimizationHorizon']]
    #param['prices']['power'].insert(0,0)

    param['prices']['power'] = np.empty(param['controlParameters']['optimizationHorizon']+1)
    print(param['prices']['power'].size)
    for i in range(0,param['prices']['power'].size-1):
    #for i in range(0,param['controlParameters']['optimizationHorizon']):
       param['prices']['power'][i+1] = powerPriceHourly[i]

    param['prices']['power'][0] = 0



    cov = np.zeros((param['controlParameters']['optimizationHorizon']+1,param['controlParameters']['optimizationHorizon']+1))
    ...
    for i in range(0,cov.shape[0]-1,24):
    ...
    for i in range(0,cov.shape[0]):
        mean[i] = param['prices']['power'][i]

    #for i in range(0,int(cov.shape[0]/24)):
    #    mean[0+i*24:(i+1)*24] = self.powerAvailable[0:24]

    np.random.seed(1)
    powerPriceSamples = np.random.multivariate_normal(mean,cov,100)

    for i in range(0,cov.shape[0]):
        for j in range(0,100):
            if powerPriceSamples[j][i] <= 0:
                powerPriceSamples[j][i] = 0

            powerPriceSamples[j][0] = 0

    param['prices']['powerPriceSamples'] = powerPriceSamples
    print(powerPriceSamples)
    print(param['prices']['power'])

    x = list(range(0,param['controlParameters']['optimizationHorizon']+1))
    for i in range(0,100):
        plt.plot(x,powerPriceSamples[i],'--r')

    plt.plot(x,param['prices']['power'],'-b',linewidth=2)
    plt.show()
