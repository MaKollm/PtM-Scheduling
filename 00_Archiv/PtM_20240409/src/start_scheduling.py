...
timeLimit = 200                            # Time limit for the solver to terminate in seconds
...
def funcStartOptimization(args, argWorkflow):
    bUseCalcCharMap = argWorkflow[0]
    print(argWorkflow)
    if len(argWorkflow) == 1:
        weekIteration = 25
    else:
        weekIteration = argWorkflow[1]
    ...
    pv.funcUpdate(param, weekIteration)
    pp.funcUpdate(param, weekIteration)
...
def main(argWorkflow):
    dataOpt = funcStartOptimization(args, argWorkflow)
