...
class Simulation():
    ...
    def funcSimulate(self, resultOpt, cm, elec, pv, pp):
        ...
        for i in range(0,self.param.param['controlParameters']['numTimeStepsToSimulate']+1):#self.time:
            ...
            self.dictOutput['storageH2Pressure'].append((self.param.param['storageH2']['InitialFilling']
                + gp.quicksum(resultOpt.dictResult['input']['powerElectrolyserRaw'][l,n] * resultOpt.dictResult['input']['modeElectrolyserRaw'][l,n,3] * self.param.param['electrolyser']['constant'] * fTimeStep for l in self.time[0:i+1] for n in elec.arrEnapterModules)
                - gp.quicksum(cm.massFlowHydrogenIn[(j)] * resultOpt.dictResult['input']['operationPoint_ABSDES'][l,j] * fTimeStep for j in cm.arrOperationPointsABSDES for l in self.time[0:i+1]) 
                - gp.quicksum(cm.massFlowAdditionalHydrogen[(j)] * resultOpt.dictResult['input']['operationPoint_ABSDES'][l,j] * fTimeStep for j in cm.arrOperationPointsABSDES for l in self.time[0:i+1])) 
            ...
            self.dictOutput['storageMethanolWaterFilling'].append(self.param.param['storageMethanolWater']['InitialFilling'] 
                + gp.quicksum(cm.massFlowMethanolWaterStorageInSynthesis[(j)] / self.param.param['storageMethanolWater']['InitialDensity'] * resultOpt.dictResult['input']['operationPoint_MEOHSYN'][l,j] * fTimeStep for j in cm.arrOperationPointsMEOHSYN for l in self.time[0:i+1]) 
                - gp.quicksum(cm.massFlowMethanolWaterStorageOut[(j)] / self.param.param['storageMethanolWater']['InitialDensity']  * resultOpt.dictResult['input']['operationPoint_DIS'][l,j] * fTimeStep for j in cm.arrOperationPointsDIS for l in self.time[0:i+1]))

            self.dictOutput['storageSynthesisgasPressure'].append((self.param.param['storageSynthesisgas']['InitialFilling']
                + gp.quicksum(cm.massFlowSynthesisgasIn[(j)] * resultOpt.dictResult['input']['operationPoint_ABSDES'][l,j] * fTimeStep for j in cm.arrOperationPointsABSDES for l in self.time[0:i+1])
                - gp.quicksum(cm.massFlowSynthesisgasOut[(j)] * resultOpt.dictResult['input']['operationPoint_MEOHSYN'][l,j] * fTimeStep for j in cm.arrOperationPointsMEOHSYN for l in self.time[0:i+1]))
