...
class Results():
    ...
    def input(self):
        ...
        if m.Status == GRB.OPTIMAL or m.Status == GRB.TIME_LIMIT:
            ...
            for i in self.time:
                for j in cf.hydrogenInValues:
                    for k in cf.biogasInValues:
                        if solution[i,j,k] == 1:
                            self.result['input']['hydrogenInput'].append(cf.hydrogenInValuesConversion[j])
                            self.result['input']['biogasInput'].append(cf.biogasInValuesConversion[k])
            ...
            if param['controlParameters']['uncertainty'] == True:
                ...
                solution = m.getAttr('X', OptModel.indicatorPV)
                for i in range(0,pv.uncertaintySamples):
    ...
    def output(self):
        ...
        for i in time:
            ...
            self.result['output']['storageH2Pressure'].append((param['storageH2']['InitialFilling']
                        ...
                        - gp.quicksum(cf.massFlowHydrogenIn[(j,k)] * self.result['input default']['operationPoint_ABSDES'][(l,j,k)] for j in cf.hydrogenInValues for k in cf.biogasInValues for l in time[0:i+1]) 
                        - gp.quicksum(cf.massFlowAdditionalHydrogen[(j,k)] * self.result['input default']['operationPoint_ABSDES'][(l,j,k)] for j in cf.hydrogenInValues for k in cf.biogasInValues for l in time[0:i+1])) 
            ...
            self.result['output']['storageSynthesisgasPressure'].append((param['storageSynthesisgas']['InitialFilling']
                + gp.quicksum(cf.massFlowSynthesisgasIn[(j,k)] * self.result['input default']['operationPoint_ABSDES'][(l,j,k)] for j in cf.hydrogenInValues for k in cf.biogasInValues for l in time[0:i+1])
            ...
            self.result['output']['massFlowSynthesisgasIn'].append(gp.quicksum(cf.massFlowSynthesisgasIn[(j,k)] * self.result['input default']['operationPoint_ABSDES'][(i,j,k)] for j in cf.hydrogenInValues for k in cf.biogasInValues))
            ...
            self.result['output']['moleFractionMethaneBiogasOut'].append(gp.quicksum(cf.moleFractionMethaneBiogasOut[(j,k)] * self.result['input default']['operationPoint_ABSDES'][(i,j,k)] for j in cf.hydrogenInValues for k in cf.biogasInValues))
            self.result['output']['powerPlantABSDES_MEOHSYN'].append(gp.quicksum(cf.powerPlantComponentsUnit1[(j,k)] * self.result['input default']['operationPoint_ABSDES'][(i,j,k)] for j in cf.hydrogenInValues for k in cf.biogasInValues))
            ...
            self.result['output']['massFlowMeOHWaterInCycle'].append(gp.quicksum(cf.massFlowMethanolWaterInCycle[(j,k)] * self.result['input default']['operationPoint_ABSDES'][(i,j,k)] for j in cf.hydrogenInValues for k in cf.biogasInValues))

            self.result['output']['volumeBiogasIn'].append(gp.quicksum(cf.volumeBiogasIn[(j,k)] * self.result['input default']['operationPoint_ABSDES'][(i,j,k)] for j in cf.hydrogenInValues for k in cf.biogasInValues))
            self.result['output']['volumeBiogasOut'].append(gp.quicksum(cf.volumeBiogasOut[(j,k)] * self.result['input default']['operationPoint_ABSDES'][(i,j,k)] for j in cf.hydrogenInValues for k in cf.biogasInValues))

            self.result['output']['moleFractionH2Synthesisgas'].append(gp.quicksum(cf.moleFractionHydrogenSynthesisgas[(j,k)] * self.result['input default']['operationPoint_ABSDES'][(i,j,k)] for j in cf.hydrogenInValues for k in cf.biogasInValues))
            self.result['output']['moleFractionCO2Synthesisgas'].append(gp.quicksum(cf.moleFractionCarbondioxideSynthesisgas[(j,k)] * self.result['input default']['operationPoint_ABSDES'][(i,j,k)] for j in cf.hydrogenInValues for k in cf.biogasInValues))
