...
class Results():
    ...
    def update(self, param):
        ...
        self.time = list(range(0,self.param['controlParameters']['numberOfTimeSteps']+1))
    ...
    def get_results(self, OptModel, cf, elec, pv, pp):
        ...
        self.output(OptModel, cf, elec, pv, pp)
        self.opt_result(OptModel, cf, elec, pv, pp)
        self.save_data(OptModel, cf, elec, pv, pp)
        #self.visualization(OptModel, cf, elec, pv, pp)
    ...
    def input(self, OptModel, cf, elec, pv, pp):
        ...
        if m.Status == GRB.OPTIMAL or m.Status == GRB.TIME_LIMIT:
            ...
            if param['controlParameters']['uncertaintyPP'] == False:
                for i in self.time:
                    self.result['input']['powerBought'].append(solution[i])
            
            else:
                for i in range(0,pp.numberUncertaintySamples):
                    self.result['input']['powerBought'].append([])
                    for j in self.time:
                       self.result['input']['powerBought'][i].append(solution[j,i])

            print(self.result['input']['powerBought'][0])
        """
        ...
        for key in self.result['input']:
            ...
            print('----------')
        """
        #print(self.result['input']['synthesisgasInMethanolSynthesis'])
        print(self.result['input']['powerBought'])

    
    def output(self, OptModel, cf, elec, pv, pp):
        param = self.param
        time = self.time
        timeStep = param['controlParameters']['timeStep']
        ...
        for i in time:
            ...
            self.result['output']['storageH2Pressure'].append((param['storageH2']['InitialFilling']
                        + gp.quicksum(self.result['input default']['powerElectrolyser'][(l,n)] * self.result['input default']['electrolyserMode'][(l,n,3)] * param['electrolyser']['constant'] * timeStep for l in time[0:i+1] for n in elec.enapterModules)
                        - gp.quicksum(cf.massFlowHydrogenIn[(j)] * self.result['input default']['operationPoint_ABSDES'][(l,j)] * timeStep for j in cf.ABSDESValues for l in time[0:i+1]) 
                        - gp.quicksum(cf.massFlowAdditionalHydrogen[(j)] * self.result['input default']['operationPoint_ABSDES'][(l,j)] * timeStep for j in cf.ABSDESValues for l in time[0:i+1])) 
            ...
            self.result['output']['storageMethanolWaterFilling'].append(param['storageMethanolWater']['InitialFilling'] 
                + gp.quicksum(cf.massFlowMethanolWaterStorageInSynthesis[(j)] / param['storageMethanolWater']['InitialDensity'] * self.result['input default']['operationPoint_MEOHSYN'][(l,j)] * timeStep for j in cf.synthesisgasInMethanolSynthesisValues for l in time[0:i+1]) 
                - gp.quicksum(cf.massFlowMethanolWaterStorageOut[(j)] / param['storageMethanolWater']['InitialDensity']  * self.result['input default']['operationPoint_DIS'][(l,j)] * timeStep for j in cf.methanolWaterInDistillationValues for l in time[0:i+1]))

            self.result['output']['storageSynthesisgasPressure'].append((param['storageSynthesisgas']['InitialFilling']
                + gp.quicksum(cf.massFlowSynthesisgasIn[(j)] * self.result['input default']['operationPoint_ABSDES'][(l,j)] * timeStep for j in cf.ABSDESValues for l in time[0:i+1])
                - gp.quicksum(cf.massFlowSynthesisgasOut[(j)] * self.result['input default']['operationPoint_MEOHSYN'][(l,j)] * timeStep for j in cf.synthesisgasInMethanolSynthesisValues for l in time[0:i+1]))
                * param['R_Synthesisgas'] * (param['Tamb'] + param['T0']) / (param['storageSynthesisgas']['Volume'] * 100000))    

            self.result['output']['massFlowSynthesisgasIn'].append(gp.quicksum(cf.massFlowSynthesisgasIn[(j)] * self.result['input default']['operationPoint_ABSDES'][(i,j)] * timeStep for j in cf.ABSDESValues))
            self.result['output']['massFlowMethanolOut'].append(gp.quicksum(cf.massFlowMethanolOut[(j)] * self.result['input default']['operationPoint_DIS'][(i,j)] * timeStep for j in cf.methanolWaterInDistillationValues))
            ...
            self.result['output']['powerPlantABSDES_MEOHSYN'].append(gp.quicksum(cf.powerPlantComponentsUnit1[(j)] * self.result['input default']['operationPoint_ABSDES'][(i,j)] * timeStep for j in cf.ABSDESValues))
            self.result['output']['powerPlantMEOHSYN'].append(gp.quicksum(cf.powerPlantComponentsUnit2[(j)] * self.result['input default']['operationPoint_MEOHSYN'][(i,j)] * timeStep for j in cf.synthesisgasInMethanolSynthesisValues))
            self.result['output']['powerPlantDIS'].append(gp.quicksum(cf.powerPlantComponentsUnit3[(j)] * self.result['input default']['operationPoint_DIS'][(i,j)] * timeStep for j in cf.methanolWaterInDistillationValues))        
            ...
            self.result['output']['volumeBiogasIn'].append(gp.quicksum(cf.volumeBiogasIn[(j)] * self.result['input default']['operationPoint_ABSDES'][(i,j)] * timeStep for j in cf.ABSDESValues))
            self.result['output']['volumeBiogasOut'].append(gp.quicksum(cf.volumeBiogasOut[(j)] * self.result['input default']['operationPoint_ABSDES'][(i,j)] * timeStep for j in cf.ABSDESValues))
        ...
        if param['controlParameters']['uncertaintyPP'] == False:
            ...
            temp2 = []
            for i in time:
                temp1.append(np.mean(pp.powerPriceSamples[:,i]))
                temp = 0
                for j in range(0,pp.numberUncertaintySamples):
                    temp = temp + self.result['input']['powerBought'][j][i]
                    
                temp2.append(temp / pp.numberUncertaintySamples)
                print(np.mean(self.result['input']['powerBought'][i]))

            print(temp2)
            print(len(temp2))
            self.result['costs']['powerBought'] = np.sum(np.dot(temp1, temp2))

        self.result['costs']['powerBatterySold'] = 0.0
        for i in time:
            self.result['costs']['powerBatterySold'] = self.result['costs']['powerBatterySold'] - param['prices']['powerSold']*self.result['input']['powerOutBatterySold'][i]
    ...
    def opt_result(self, OptModel, cf, elec, pv, pp):
        m = OptModel.m
    ...
    def save_data(self, OptModel, cf, elec, pv, pp):
        ...
        savemat("data_param_" + string + ".mat", self.param)
