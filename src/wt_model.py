...
class WT():
	...
	def __init__(self,param):
		...
		self.considerWT = param.param['controlParameters']['considerWT']

		self.pathPVData = param.param['controlParameters']['pathPVData'] #maybe change if different csv is used

		self.online_weather_data = None
		self.online_weather_data_noisy = None
		self.csv_weather_data = None #not implemented


	#update parameters of WT
	...
	def funcGetOnlineData(self):
		...
		hourly_temperature_120m = hourly.Variables(4).ValuesAsNumpy()
		hourly_temperature_180m = hourly.Variables(5).ValuesAsNumpy()
